  ...
  if (!transaction) {
    return (
      <SafeAreaView className="flex-1 bg-white">
        <Header iconLeft="chevron-left" onPressIconLeft={onPressBack} title="Chi tiết" />
        <View className="flex-1 justify-center items-center">
          <Text className="text-gray-500">Không tìm thấy thông tin giao dịch.</Text>
        </View>
      </SafeAreaView>
    );
  }
      ...
      <View className="flex-1">
        ...
        <ScrollView className="flex-1 px-4 mt-4" showsVerticalScrollIndicator={false}>
          ...
          <View className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm items-center mb-6">
            <Text className="text-gray-500 text-sm mb-2 uppercase tracking-widest">Tổng thanh toán</Text>
            <Text className="text-4xl font-bold text-blue-600 mb-4">
              {formatCurrency(transaction.amount)}
            </Text>
            
            <View className={`px-4 py-2 rounded-full border ${statusInfo.bg} ${statusInfo.border}`}>
          ...
          <View className="bg-gray-50 p-5 rounded-2xl space-y-4 gap-4 mb-6">
            <Text className="text-lg font-bold text-gray-800 mb-2">Thông tin đơn hàng</Text>
